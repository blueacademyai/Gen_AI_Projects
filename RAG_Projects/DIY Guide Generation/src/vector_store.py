...
class SupportVectorStore:
    ...
    def process_metadata_for_return(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def create_vector_store(self, documents_by_type: Dict[str, List[Document]]) -> None:
        ...
        for support_type, docs in documents_by_type.items():
            ...
            texts = [doc.page_content for doc in docs]  # This contains only subject and body
            # metadatas = [doc.metadata for doc in docs]
            metadatas = [self.prepare_metadata(doc.metadata) for doc in docs]
    ...
    def load_local(cls, directory: str) -> 'SupportVectorStore':
        ...
        Returns:
            SupportVectorStore: Loaded vector store instance or None if no collections found
        """
        ...
        for collection in collections:
            ...
            if support_type:
                instance.collections[support_type] = collection_obj

        if instance.collections:    
            logger.info(f"Loaded vector store from {directory} with {len(instance.collections)} collections")
            return instance
        else:
            logger.warning(f"No valid collections found in {directory}")
            return None
    ...
    def query_similar(
        ...
        for collection in collections_to_query:
            ...
            for doc, metadata, distance in zip(
                ...
                results.append({
                    'content': doc,
                    'metadata': self.process_metadata_for_return(metadata),
